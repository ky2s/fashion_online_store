...
class ControllerCatalogCategory extends Controller {
	...
	public function add() {
		...
		if (isset($this->request->post['category_description'])) {
			foreach ($this->request->post['category_description'] as $language_id => $value) {
				$this->request->post['category_description'][$language_id]['meta_title'] = $value['name'];					
				$cName = $value['name'];
				$this->request->post['keyword'] = hyphenize(htmlspecialchars_decode($cName));	
			}			
		}
	...
	protected function validateForm() {
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			...
			do {
				...
				if(($url_alias_info && isset($this->request->get['category_id']) && $url_alias_info['query'] != 'category_id=' . $this->request->get['category_id']) || ($url_alias_info && !isset($this->request->get['category_id']))) {
					$str = explode('-',$this->request->post['keyword']);
					$last = end($str);

					if ((int)$last != 0) {
					    $key = key($str);  
					    $str[$key] = (int)$last + 1;
					    $this->request->post['keyword'] = implode($str,'-');					    
					} else {
					    $this->request->post['keyword'] .= '-1';
					}
					
				} else {
					break;
				}				
			} while ( $url_alias_info );
	...
	public function getparents() {
		...
		if (isset($this->request->get['parrent_id'])) {

			$this->load->model('catalog/category');

			$results = $this->model_catalog_category->getCategoryParrents($this->request->get['parrent_id']);
