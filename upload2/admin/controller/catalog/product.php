	...
	public function add() {
		...
		if (isset($this->request->post['product_description'])) {
			foreach ($this->request->post['product_description'] as $language_id => $value) {
				$this->request->post['product_description'][$language_id]['meta_title'] = $value['name'];
				$pName = $value['name'];			
			}
		}

		if (isset($this->request->post['model'])) {
			$this->request->post['keyword'] = hyphenize(htmlspecialchars_decode($pName));	
		}
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('catalog/productmanager', 'token=' . $this->session->data['token'] . $url, 'SSL'));
		}
	...
	protected function validateForm() {
		...
		if (utf8_strlen($this->request->post['keyword']) > 0) {
			...
			do {
				...
				if(($url_alias_info && isset($this->request->get['product_id']) && $url_alias_info['query'] != 'product_id=' . $this->request->get['product_id']) || ($url_alias_info && !isset($this->request->get['product_id']))) {					
