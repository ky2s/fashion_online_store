...
class ModelMarketingCoupon extends Model {
	...
	public function getCoupons($data = array()) {
		$sql = "SELECT coupon_id, name, code, discount, date_start, date_end, status FROM " . DB_PREFIX . "coupon WHERE newsletter_id IS NULL";
	...
	public function getTotalCoupons() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "coupon WHERE newsletter_id IS NULL");
	...
	public function getCouponHistoriesNewsletter($start = 0, $limit = 10) {
		if ($start < 0) {
			$start = 0;
		}
		...
		$sql = "SELECT co.coupon_id, 
		                 co.name, 
		                 co.code,
		                 co.discount,
		                 co.date_start as coupon_added,
		                 co.date_end as coupon_end,
		                 ch.order_id,
		                 ...
		                 n.newsletter_gender,
		                 n.newsletter_status
		                 FROM " . DB_PREFIX . "coupon co
		                 LEFT JOIN " . DB_PREFIX . "coupon_history ch ON (co.coupon_id = ch.coupon_id)
		                 LEFT JOIN " . DB_PREFIX . "customer c ON (ch.customer_id = c.customer_id)
		                 LEFT JOIN " . DB_PREFIX . "newsletter n ON (n.newsletter_id = co.newsletter_id)
		                 WHERE co.newsletter_id IS NOT NULL ORDER BY co.date_added DESC LIMIT " . (int)$start . "," . (int)$limit;
	...
	public function getTotalCouponHistoriesNewsletter() {
		$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "coupon WHERE newsletter_id IS NOT NULL");
