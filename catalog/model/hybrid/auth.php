...
class ModelHybridAuth extends Model {
    ...
    public function addCustomer($data) {
        ...
        if (isset($this->request->cookie['tracking'])) {
            ...
            $affiliate_info = $this->model_affiliate_affiliate->getAffiliateByCode($tracking_code);
            if ($affiliate_info) {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "customer SET store_id = '" . (int)$this->config->get('config_store_id') . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', password = '" . $this->db->escape(md5($data['password'])) . "', gender='{$gender}', newsletter = '" . (isset($data['newsletter']) ? (int)$data['newsletter'] : 0) . "', customer_group_id = '" . (int)$this->config->get('config_customer_group_id') . "', status = '1', date_added = NOW(), affiliate_id = '{$affiliate_id}', affiliate_name = '{$affiliate_name}', register_method = '{$register_method}', birth_date = '{$birth_date}'");

        $customer_id = $this->db->getLastId();
        // Enable newsletter by default
        $this->db->query("INSERT INTO " . DB_PREFIX . "newsletter SET newsletter_email = '" . $this->db->escape($data['email']) . "', newsletter_gender='{$gender}', newsletter_add_date = NOW(), newsletter_status=1");
