...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false,$post_data=array(), $tracking_session=false) {
		...
		if ($order_info) {
			...
			if(! empty($post_data['nomor_resi'])){

				$sql .= " ,nomor_resi='{$post_data['nomor_resi']}' ";
			...
			if(! empty($post_data['added_by'])){

				$admin_id = ", added_by = '{$post_data['added_by']}'";

			}



			$this->db->query("INSERT INTO " . DB_PREFIX . "order_history SET order_id = '" . (int)$order_id . "', order_status_id = '" . (int)$order_status_id . "', notify = '" . (int)$notify . "', comment = '" . $this->db->escape($comment) . "', date_added = NOW() {$admin_id}");
			...
			if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
				...
				unset($this->session->data['referral_point']);
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$data['full_name'] = $order_info['firstname'];
				...
				if ($this->config->get('config_order_mail')) {
					...
					//send to main admin when in influencer store
					...
					if($this->config->get('config_store_id') != 0){
						...
						$main_store = $this->model_setting_setting->getSetting('config');

						

						$mail = new Mail($this->config->get('config_mail'));

						$mail->setTo($main_store['config_email']);

						$mail->setFrom($main_store['config_email']);
				...
				//Notif to Affiliate user

				/*if ($tracking_session) {

					$subject = sprintf($language->get('text_new_subject'), html_entity_decode($this->config->get('config_name'), ENT_QUOTES, 'UTF-8'), $order_id .'(Affiliate)');
			...
			if ($order_info['order_status_id'] && $order_status_id) {
				...
				$data['text_update_order'] 	    = $language->get('text_update_order');

				$data['order_id']               = $order_id;

				$data['base']                   = $base;

				$data['text_update_date_added'] = $language->get('text_update_date_added');
				...
				if ($order_info['customer_id']) {
					...
					$data['update_link'] = $order_info['store_url'] . 'index.php?route=account/order/info&order_id=' . $order_id;

				} else {

					$data['text_update_link'] = '';
				...
				if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/mail/orderUpdate.tpl')) {

					$html = $this->load->view($this->config->get('config_template') . '/template/mail/orderUpdate.tpl', $data);

				} else {

					$html = $this->load->view('default/template/mail/orderUpdate.tpl', $data);
