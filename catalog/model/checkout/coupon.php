...
class ModelCheckoutCoupon extends Model {
	public function getCoupon($code,$order_info=array()) {
		$status = true;
		...
		if ($coupon_query->num_rows) {
			...
			// Coupon store
			$store_ids = array();
			$coupon_store_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "coupon_to_store` cs WHERE cs.coupon_id = '" . (int)$coupon_query->row['coupon_id'] . "'");

			foreach ($coupon_store_query->rows as $store) {
			...
			if (! $store_ids) {
				...
				if($order_info){
					if(! in_array($order_info['store_id'], $store_ids)){
						$status = false;
					}
				}else{
					if(! in_array((int)$this->config->get('config_store_id'), $store_ids)){
						$status = false;
					}
				}
			}
