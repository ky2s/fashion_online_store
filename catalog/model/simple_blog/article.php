...
class ModelSimpleBlogArticle extends Model {
	...
	public function getTotalArticle($data = array()) {
		...
		$sql = "
		SELECT COUNT(DISTINCT(sba.simple_blog_article_id)) AS total 
		FROM `" . DB_PREFIX . "simple_blog_article` sba 
		...
		if(!empty($data['year'])) {
			$sql .= " AND YEAR(sba.date_post) = '" . $this->db->escape($data['year'])."'";
		}
		
		if(!empty($data['month'])) {
			$sql .= " AND MONTH(sba.date_post) = '" . $this->db->escape($data['month'])."'";
		}
	...
	public function getArticles($data = array()) {
		...
		// 	SELECT 
		// 		sba.*, 
		// 		sbcd.`simple_blog_category_id`,
		// 		sbcd.`name` AS category_name,
		// 		sbad.*, 
		// 		sbau.name AS author_name 
		// 	FROM 
		// 		`" . DB_PREFIX . "simple_blog_article` sba 
		// 	LEFT JOIN 
		// 		`" . DB_PREFIX . "simple_blog_article_description` sbad 
		// 			ON (sba.simple_blog_article_id=sbad.simple_blog_article_id) 
		// 	LEFT JOIN 
		// 		`" . DB_PREFIX . "simple_blog_article_to_store` sbas 
		// 			ON(sba.simple_blog_article_id=sbas.simple_blog_article_id) 
		// 	LEFT JOIN 
		// 		`" . DB_PREFIX . "simple_blog_author` sbau 
		// 			ON(sba.simple_blog_author_id=sbau.simple_blog_author_id) 
		// 	LEFT JOIN 
		// 		`" . DB_PREFIX . "simple_blog_article_to_category` sbac 
		// 			ON(sba.simple_blog_article_id=sbac.simple_blog_article_id) 
		// 	LEFT JOIN 
		// 		`" . DB_PREFIX . "simple_blog_category_description` sbcd 
		// 			ON(sbac.simple_blog_category_id=sbcd.simple_blog_category_id) 
		// 	WHERE 
		// 		sba.status=1 
		// 		AND sbau.status=1 
...
/*			
		if($data['archive_data']){
			$sql .= " ,COUNT(sba.simple_blog_article_id) as archive_total";
		}
*/
		$sql .=" FROM 
				`" . DB_PREFIX . "simple_blog_article` sba 
		...
		if(!empty($data['prev_article'])) {
			$sql .= " AND sba.simple_blog_article_id < '" . $this->db->escape($data['prev_article'])."'";
		}

		if(!empty($data['next_article'])) {
			$sql .= " AND sba.simple_blog_article_id > '" . $this->db->escape($data['next_article'])."'";
		}
		...
		if($data['archive_data']){
			$sql .= "GROUP BY YEAR(sba.date_post)";
			//$group_by .= " ,YEAR(sba.date_added)";
		}
		else{
			$sql .= "GROUP BY sba.simple_blog_article_id";	
		}


		if(!empty($data['next_article'])){
			$sql .= " ORDER BY (sba.date_post) ASC";
		}else{
			$sql .= " ORDER BY (sba.date_post) DESC";	
		}
	...
	public function getArticle($simple_blog_article_id) {
		...
		// 			ON(sbac.simple_blog_category_id=sbcd.simple_blog_category_id) 
		// 	WHERE 
		// 		sba.simple_blog_article_id='" . (int)$simple_blog_article_id . "' 
		// 		AND sbau.status=1 
		// 		AND sba.status=1 
		// 		AND sbas.store_id='" . (int)$this->config->get('config_store_id') . "' 
		...
		$sql = $this->db->query("
			...
			LEFT JOIN 
				`" . DB_PREFIX . "simple_blog_category_description` sbcd 
					ON(sbac.simple_blog_category_id=sbcd.simple_blog_category_id)
			LEFT JOIN 
				`" . DB_PREFIX . "simple_blog_article_description_additional` sbada
					ON(sba.simple_blog_article_id = sbada.simple_blog_article_id)
	...
	public function getCommentsByArticle($simple_blog_article_id, $start = 0, $limit = 20, $simple_blog_comment_id = 0) {
		...
		if(!$simple_blog_comment_id) {
			...
			$test = "SELECT * FROM `" . DB_PREFIX . "simple_blog_comment` WHERE simple_blog_article_id='" . (int)$simple_blog_article_id . "' AND status=1 AND simple_blog_article_reply_id='0' ORDER BY date_added DESC LIMIT " . (int)$start . "," . (int)$limit;
	...
	public function addArticleComment($simple_blog_article_id, $data) {
		...
		if($data['reply_id']) {
			...
			$this->db->query("INSERT INTO `" . DB_PREFIX . "simple_blog_comment` SET simple_blog_article_id='" . (int)$simple_blog_article_id . "', simple_blog_article_reply_id='" . (int)$data['reply_id'] . "', author='" . $this->db->escape($data['name']) . "', user_id='" . $this->db->escape($data['user_id']) . "', comment='" . $this->db->escape($data['text']) . "', status='" . (int)$status . "', date_added=NOW(), date_modified=NOW()");
		} else {
			$this->db->query("INSERT INTO `" . DB_PREFIX . "simple_blog_comment` SET simple_blog_article_id='" . (int)$simple_blog_article_id . "', author='" . $this->db->escape($data['name']) . "', user_id='" . $this->db->escape($data['user_id']) . "', comment='" . $this->db->escape($data['text']) . "', status='" . (int)$status . "', date_added=NOW(), date_modified=NOW()");
		}
	}
		...
		$sql = "
			...
			LEFT JOIN 
				`" . DB_PREFIX . "simple_blog_category_description` sbcd 
					ON(sbac.simple_blog_category_id=sbcd.simple_blog_category_id) 
			WHERE 
				sbac.simple_blog_category_id='" . (int)$data['simple_blog_article_id'] . "' 
				AND sba.status=1 
				AND sbau.status=1 
				AND sbas.store_id='" . (int)$this->config->get('config_store_id') . "' 
				AND sbad.language_id='" . $this->config->get('config_language_id') . "' 
			ORDER BY 
				sba.date_modified DESC
		";
	...
	public function getRelatedArticles($simple_blog_article_id) {
		...
		foreach($sql->rows as $row) {
			$article_info = $this->db->query("SELECT sba.*, sbad.article_title AS article_title, sbad.description AS description, sbau.simple_blog_author_id AS simple_blog_author_id, sbau.name AS author_name, sbcd.name AS article_category FROM `" . DB_PREFIX . "simple_blog_article` sba LEFT JOIN `" . DB_PREFIX . "simple_blog_article_description` sbad ON(sba.simple_blog_article_id=sbad.simple_blog_article_id) LEFT JOIN `" . DB_PREFIX . "simple_blog_author` sbau ON(sba.simple_blog_author_id=sbau.simple_blog_author_id) LEFT JOIN 
				`" . DB_PREFIX . "simple_blog_article_to_category` sbac 
					ON(sba.simple_blog_article_id=sbac.simple_blog_article_id) 
			LEFT JOIN 
				`" . DB_PREFIX . "simple_blog_category_description` sbcd 
					ON(sbac.simple_blog_category_id=sbcd.simple_blog_category_id) WHERE sba.simple_blog_article_id='" . (int)$row['simple_blog_article_related_id'] . "' AND sbad.language_id='" . (int)$this->config->get('config_language_id') . "' AND sba.status=1 AND sbau.status=1");
			//pre($article_info);
			if($article_info->row) {
				
				$total_comment = $this->getTotalComments($row['simple_blog_article_related_id']);
				
				$image = $this->model_tool_image->resize($article_info->row['featured_image'], 263, 180);
				...
				$simple_blog_related_article_data[] = array(
					...
					'article_category'		=> $article_info->row['article_category'],
				);	
			}				
		}
	...
	public function getBannedWord(){
		$sql = $this->db->query("SELECT * FROM `" . DB_PREFIX . "banned_word` ");
		return $sql->rows;
	}

	public function getMonth($year){
		$sql = $this->db->query("SELECT MONTHNAME(STR_TO_DATE(MONTH(sba.date_post), '%m')) as month_name, count(sba.simple_blog_article_id) as total FROM `" . DB_PREFIX . "simple_blog_article` sba WHERE YEAR(sba.date_post) = {$year} GROUP BY MONTH(sba.date_post)");
		return $sql->rows;
	}	
