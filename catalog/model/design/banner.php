...
class ModelDesignBanner extends Model {
	...
	public function getBannerActive($banner_id) {
		$query = $this->db->query("
			...
			WHERE b.status = 1 AND  bi.banner_id = '" . (int)$banner_id . "' 
	...
	public function getBannerSteph($banner_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "banner b 
			                       LEFT JOIN " . DB_PREFIX . "banner_image bi ON (bi.banner_id  = b.banner_id)
			                       LEFT JOIN " . DB_PREFIX . "banner_image_description bid ON (bi.banner_image_id  = bid.banner_image_id) WHERE b.status = 1 AND b.banner_id = '" . (int)$banner_id . "' AND bid.language_id = '" . (int)$this->config->get('config_language_id') . "' ORDER BY bi.sort_order ASC");
