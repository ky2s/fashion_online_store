...
*/
	...
	public function getOrders($start = 0, $limit = 20,$filter=array()) {
	    $_limit="";
		if ($limit) {
		    if($start){
		      $_limit = " LIMIT " . (int)$start . "," . (int)$limit;  
		    }else{
        ...
        if(! empty($filter['order_status_id'])){
            $where .= " AND o.order_status_id = {$filter['order_status_id']}";
        }

		if(!empty($filter['order_last_date'])){
            $where .= " AND o.date_added >= (NOW() - INTERVAL 1 MONTH)";  //where 1 last month    
        }        
        ...
        /*pre("SELECT 
            ...
            AND 
                o.order_status_id > '0' 
            AND 
                o.store_id = '" . (int)$this->config->get('config_store_id') . "' 
            AND 
