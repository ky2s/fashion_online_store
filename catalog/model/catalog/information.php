...
class ModelCatalogInformation extends Model {
	...
	public function getLimitInformations($limit_offset) {
		
		if ($limit_offset) {
			$limit_offset = "LIMIT ".$limit_offset['limit'].",".$limit_offset['offset'];
		}else{
			$limit_offset = '';
		}		
		...
		$query = $this->db->query("
			...
			AND i2s.store_id = '" . (int)$this->config->get('config_store_id') . "' 
			AND i.status = '1' 
			ORDER BY i.sort_order, LCASE(id.title) ASC ".$limit_offset);
