<?php if($comments) { ?>
    <?php foreach ($comments as $comment) { ?>
		<div class="article-reply form-group">
  			...
  			<div><a id="reply-comment" name="#write-comment" target="write-comment" onclick="setArticleId(<?php echo $comment['simple_blog_comment_id']; ?>);" style="cursor: pointer;"><?php echo $text_reply_comment; ?></a></div>
  			<br />
...
<script type="text/javascript">
	function setArticleId(article_id) {
	   ...
	   $('html, body').animate({
            scrollTop: $("#write-comment").offset().top - 50
        }, 1000);
  }
</script>