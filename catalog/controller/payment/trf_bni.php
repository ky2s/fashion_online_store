...
class ControllerPaymentTrfBni extends Controller {
	...
	public function confirm() {
		if ($this->session->data['payment_method']['code'] == 'trf_bni') {
			...
			$this->session->data['payment_number'] = $this->config->get('trf_bni_accountno');
			$this->session->data['payment_name'] = $this->config->get('trf_bni_accountname');
			...
			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('trf_bni_order_status_id'), $comment, true, $this->session->data['tracking']);
		}		
	}
}
?>