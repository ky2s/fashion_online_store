...
class ControllerPaymentVeritrans extends Controller {
  ...
  public function process_order() {
    ...
    //save to session 
    $this->session->data['payment_detail1'] = NULL;
    $this->session->data['payment_logo'] = NULL;
    $this->session->data['payment_number'] = NULL;
    $this->session->data['payment_name'] = NULL;
    ...
    //ADD TO HISTORY
    $comment  = $this->language->get('text_title') . "\n" . $this->language->get('text_detail') . "\n\n";
    //$comment .= $this->language->get('text_detail');

    $this->model_checkout_order->addOrderHistory($this->session->data['order_id'],
    $this->config->get('veritrans_vtweb_challenge_mapping'), $comment, true, $this->session->data['tracking']);
    ...
    foreach ($products as $product) {
      if (($this->config->get('config_customer_price')
          ...
          || !$this->config->get('config_customer_price')) {
            ...
            //discount by Store discount (value dihitung setelah tax)
            /*$product_price = $product['price'];
            if($product_price)
            {
              $store_discount = $this->config->get('config_store_discount');
              if($store_discount){
                $discount = $product_price * $store_discount/100;
                $product_price = $product_price - $discount;
              }
            }*/

            $product_price= $this->tax->calculate($product['price'], $product['tax_class_id'],$this->config->get('config_tax'));
      }
      ...
      $item = array(
          ...
          'name'     => strlen($product['name']) > 47 ? substr($product['name'],0,47)."...":$product['name']
        );
      $item_details[] = $item;
    }
    ...
    if ($this->cart->hasShipping()) {
        ...
        if (($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) {
              ...
              if($shipping_cost)
              {
                $store_discount = $this->config->get('config_store_discount');
                if($store_discount){
                  $discount = $shipping_cost * $store_discount/100;
                  $shipping_cost = $shipping_cost - $discount;
                }
              }*/

              $shipping_info['cost'] = $this->tax->calculate($shipping_info['cost'], $product['tax_class_id'],$this->config->get('config_tax'));
        }
        ...
        if ($this->currency->has('IDR')) {
                foreach ($item_details as &$item) {
                    ...
                    if($item_price)
                    {
                        $store_discount = $this->config->get('config_store_discount');
                        if($store_discount){
                          $discount = $item_price * $store_discount/100;
            ...
            foreach ($item_details as &$item) {
                $item['price'] = intval($item['price'] * $this->config->get('veritrans_currency_conversion'));
            }
            unset($item);

            $transaction_details['gross_amount'] = intval($transaction_details['gross_amount'] * $this->config->get('veritrans_currency_conversion'));
        }
        else {
    ...
    try {
      ...
      if ($this->config->has('veritrans_enabled_payments')) {
        foreach ($this->config->get('veritrans_enabled_payments') as $key => $value) {
      ...
      //$this->response->setOutput(json_encode($payloads));
      ...
      //$this->response->setOutput(json_encode($item_details));
