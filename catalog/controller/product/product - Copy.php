...
class ControllerProductProduct extends Controller {
	...
	public function index() {
		...
		if ($product_info) {
			...
			$product_data = $this->model_catalog_product->getProductsByGroupId(null, $group_id);
			...
			foreach ($product_data as $i => $pd) {
				//option colors
				if($pd['color']){
					//$stock_status = ($pd['stock_status'] = 'Out Of Stock') ? false : true;
					$colors[] = array(
			...
			$information_id = 23; //23: ID in table information
			$guide_result = $this->model_catalog_information->getInformation($information_id);
			$data['size_guide'] = $guide_result['description'];*/

			//pre($data['products_by_group']);
			//pre($data['thumbs']);
			//pre($data['images']);
