		...
		if (!$json) {
			...
			foreach ($total_data as $result) {
				...
				if($result['code'] == 'credit'){
					$part_credit = str_replace('Credit', '', $result['title']);
					$totals[] = array(
						'code' => 'coupon',
						'title' => $part_credit,
						'value'  => $this->currency->format($result['value']),
				...
				if($result['code'] == 'shipping'){
					$part = preg_split('/[\s,]+/', $this->session->data['shipping_method']['title'], 3);
					$totals[] = array(
						'code' => 'shipping',
						'title' => $part[0]. ' ' . $part[1],
						'value'  => $this->currency->format($this->session->data['shipping_method']['cost']),
					);	
				}
