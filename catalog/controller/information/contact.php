...
class ControllerInformationContact extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($server .'contact?status=1');
		}
		...
		$this->session->data['redirect'] = $this->url->link('information/contact');
		...
		if ($this->config->get('config_image')) {
			$data['image'] = $this->model_tool_image->resize($this->config->get('config_image'), 304, $this->config->get('config_image_location_height'));
		} else {
			$data['image'] = false;
		}
		...
		$data['message'] = ! empty($this->request->get['status']) ? 'Your enquiry has been successfully sent to the store owner!' : '';
		...
		$data['aff_data'] = $this->session->data['data_affiliate'];
