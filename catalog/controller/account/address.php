...
class ControllerAccountAddress extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->model_affiliate_affiliate->editAffiliateFromCustomer($data_post);
	...
	protected function getList() {
		...
		foreach ($results as $result) {
			if ($result['address_format']) {
				...
				$format = '<div class="name-addr">{firstname} {lastname}</div>' . "" . '<div>{company}</div>' . "" . '{address_1}' . "\n" . '{address_2}' . "\n" . '{city} {postcode}' . "\n" . '{zone}' . "\n" . '{country}';
			}
	...
	protected function getForm() {
		...
		if (isset($this->request->post['name'])) {
			...
			$data['name'] = $address_info['firstname'] .' '. $address_info['lastname'];
		} else {
			$data['name'] = '';
		}
		...
		if (isset($this->request->post['city'])) {
			$data['city_id'] = $this->request->post['city'];
		} elseif (!empty($address_info)) {
        ...
        $data['cities'] = $this->model_localisation_city->getCitiesByZoneId($data['zone_id']);
    	#echo$data['city_id'];
