...
class ControllerCommonCart extends Controller {
	public function index() {
		...
		if($this->cart->countProducts() > 1){
			...
			$data['text_items'] = sprintf($this->language->get('text_item'), $this->cart->countProducts() + (isset($this->session->data['vouchers']) ? count($this->session->data['vouchers']) : 0), $this->currency->format($total));
		}
		...
		foreach ($this->cart->getProducts() as $i=>$product) {
			...
			$product_result = $this->model_catalog_product->getProductByGroup($product['product_id']);
			...
			$data['products'][] = array(
				...
				'price'     => $price,
				'total'     => $total,
				'href'      => $this->url->link('product/product', 'product_id=' . $product['product_id'] .'&group_id=' . $product_result[0]['group_id'])
			);

			$grand_totals += $product['price'];
