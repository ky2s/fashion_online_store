...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		$data['crossCheck'] = $this->url->link('sale/order/crossCheck', 'token=' . $this->session->data['token'], 'SSL');
        ...
        if(!empty($this->session->data['selected'])){
            foreach($this->session->data['selected'] as $key => $val){
    ...
    public function crossCheck(){
        if (isset($this->request->post['selected'])) {
            foreach($this->request->post['selected'] as $key => $val ){
                if($val=='false'){
                    unset($this->session->data['selected'][$key]);            
                }else{
                    $this->session->data['selected'][$key] = true;
                }
            }
        }
    }
	...
	public function info() {
		...
		if ($order_info) {
			...
			$data['column_wrap_name'] = $this->language->get('column_wrap_name');
			$data['column_wrap_price'] = $this->language->get('column_wrap_price');	
			$data['column_wrap_image'] = $this->language->get('column_wrap_image');					
			$data['column_wrap_quantity'] = $this->language->get('column_wrap_quantity');
			$data['text_subtotal'] = $this->language->get('text_subtotal');			
			$data['text_wrap_comment'] = $this->language->get('text_wrap_comment');		

			$data['column_gift_card_name'] = $this->language->get('column_gift_card_name');
			$data['column_gift_card_price'] = $this->language->get('column_gift_card_price');	
			$data['column_gift_card_image'] = $this->language->get('column_gift_card_image');					
			$data['column_gift_card_quantity'] = $this->language->get('column_gift_card_quantity');				
			$data['text_gift_card_comment'] = $this->language->get('text_gift_card_comment');	

			$data['text_order_id'] = $this->language->get('text_order_id');
			$data['tab_wrapping'] = $this->language->get('tab_wrapping');
			$data['tab_gift_card'] = $this->language->get('tab_gift_card');
			...
			//WRAP	
			$data['wrap_comment'] = nl2br($order_info['wrap_comment']);
			$data['wrap_name'] = $order_info['wrap_name'];
			$data['wrap_total'] = $this->currency->format($order_info['wrap_total'], $order_info['currency_code'], $order_info['currency_value']);
			$data['wrap_image'] = $this->model_tool_image->resize($order_info['wrap_image'], $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height'));

			//GIFT CARD	
			$data['card_comment'] = nl2br($order_info['card_comment']);
			$data['card_name'] = $order_info['card_name'];
			$data['card_total'] = $this->currency->format($order_info['card_total'], $order_info['currency_code'], $order_info['currency_value']);
			$data['card_image'] = $this->model_tool_image->resize($order_info['card_image'], $this->config->get('config_image_thumb_width'), $this->config->get('config_image_thumb_height'));
	...
	public function invoice() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
        		...
        		if ( count($orders) == 1) {
        			$data['title'] = $this->language->get('text_invoice')." #{$order_id}".($invoice_no?" - {$invoice_no}":"");
        		}
	...
	public function shipping() {
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info && $order_info['shipping_code']) {
        		...
        		if ( count($orders) == 1) {
        			$data['title'] = "DO #{$order_id}".($invoice_no?" - {$invoice_no}":"");
        		}
	...
	public function city2() {
		...
		foreach ($results as $result) {
			...
			if (isset($this->request->get['city']) && ($this->request->get['city'] == $result['name'])) {
				$output .= ' selected="selected"';
			}
