...
class ModelToolExportImport extends Model {
	...
	protected function uploadCategories( &$reader, $incremental ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			##MODIFY BY STEPH##
			$seo_keyword = hyphenize(htmlspecialchars_decode($name));	

			$this->load->model('catalog/url_alias');
			do {
				$url_alias_info = $this->model_catalog_url_alias->getUrlAlias($seo_keyword);
				if(($url_alias_info && isset($category_id) && $url_alias_info['query'] != 'category_id=' . $category_id) || ($url_alias_info && !isset($category_id))) {					
					$str = explode('-',$seo_keyword);
					$last = end($str);

					if ((int)$last != 0) {
					    $key = key($str);  
					    $str[$key] = (int)$last + 1;
					    $seo_keyword = implode($str,'-');					    
					} else {
					    $seo_keyword .= '-1';
					}
				} else {
	...
	protected function uploadProducts( &$reader, $incremental, &$available_product_ids=array() ) {
		...
		for ($i=0; $i<$k; $i+=1) {
			...
			while ($this->startsWith($first_row[$j-1],"name(")) {
				...
				$name = htmlspecialchars( $name );
				$names[$language_code] = $name;
				//$meta_titles[$language_code] = $name;
			}
			...
			##MODIFY BY STEPH##
			$keyword = hyphenize(htmlspecialchars_decode($name));	

			$this->load->model('catalog/url_alias');
			do {
				$url_alias_info = $this->model_catalog_url_alias->getUrlAlias($keyword);
				if(($url_alias_info && isset($product_id) && $url_alias_info['query'] != 'product_id=' . $product_id) || ($url_alias_info && !isset($product_id))) {					
					$str = explode('-',$keyword);
					$last = end($str);
			...
			while ($this->startsWith($first_row[$j-1],"meta_keywords(")) {
				...
				$meta_keyword = htmlspecialchars( $meta_keyword );
				$meta_keywords[$language_code] = $meta_keyword;
				//$tags[$language_code] = $meta_keyword;
			}
