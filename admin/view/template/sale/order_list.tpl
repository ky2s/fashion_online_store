        ...
        <form method="post" enctype="multipart/form-data" target="_blank" id="form-order">
          <div class="table-responsive">
            <table class="table table-bordered table-hover">
              <thead>
                <tr>
                  <td style="width: 1px;" class="text-center"><input type="checkbox" onclick="$('input[name*=\'selected\']').prop('checked', this.checked);crossCheck();" /></td>
...
function crossCheck(){
    $.ajax({            
        url: '<?php echo html_entity_decode($crossCheck); ?>',
        type: 'post',
        data: $('input[name="selected[]"]').map(function() {
            return { name: 'selected['+this.value+']', value: this.checked ? "true" : "false" };
        }),            
    }); 
}

$(function(){
	$('#form-order').on('change','input[name="selected[]"]',function(){
	     crossCheck();
	});
});
