              ...
              <div class="form-group required">
                ...
                <div class="col-sm-10">
                  <select name="city" id="input-city" class="form-control" data-live-search="true">
                    <option value=""><?php echo $text_select; ?></option>
                    <?php foreach ($cities as $city) { ?>                    
                    <?php if ($city['name'] == $shipping_city) { ?>
                    <option value="<?php echo $city['name']; ?>" selected="selected"><?php echo $city['name']; ?></option>
                    <?php } else { ?>
                    <option value="<?php echo $city['name']; ?>"><?php echo $city['name']; ?></option>
                    <?php } ?>
...
$('#tab-payment select[name=\'country_id\']').on('change', function() {
	$.ajax({
		url: 'index.php?route=sale/order/zone2&token=<?php echo $token; ?>&country_id=' + this.value + '&zone_id='+payment_zone_id,
		dataType: 'html',			
  ...
  $('#tab-payment select[name=\'zone_id\']').on('change', function() {
    var zone_id = this.value || payment_zone_id;
	$.ajax({
		url: 'index.php?route=sale/order/city2&token=<?php echo $token; ?>&zone_id=' + zone_id +'&city='+payment_city,
		dataType: 'html',			
		success: function(html) {
...
$('#tab-shipping select[name=\'country_id\']').on('change', function() {
	$.ajax({
		url: 'index.php?route=sale/order/zone2&token=<?php echo $token; ?>&country_id=' + this.value + '&zone_id='+shipping_zone_id,
		dataType: 'html',
		success: function(html) {
  ...
  $('#tab-shipping select[name=\'zone_id\']').on('change', function() {
    var zone_id = this.value || shipping_zone_id;
	$.ajax({
		url: 'index.php?route=sale/order/city2&token=<?php echo $token; ?>&zone_id=' + zone_id +'&city='+shipping_city,
		dataType: 'html',			
		success: function(html) {
