              ...
              <div class="form-group required">
                ...
                <div class="col-sm-10">
                  <select name="city" id="input-city" class="form-control">
                    ...
                    <?php if ($value['city_id'] == $city) { ?>
                    <option value="<?php echo $value['city_id']; ?>" selected="selected"><?php echo $value['name']; ?></option>
                    <?php } else { ?>
...
$('select[name=\'zone_id\']').on('change', function() {
  $.ajax({
    url: 'index.php?route=marketing/affiliate/zone&token=<?php echo $token; ?>&zone_id=' + this.value,
    dataType: 'json',
    beforeSend: function() {
    ...
    success: function(json) {
      ...
      if (json['city'] != '') {
        for (i = 0; i < json['city'].length; i++) {
          html += '<option value="' + json['city'][i]['zone_id'] + '"';
          
          if (json['city'][i]['zone_id'] == '<?php echo $zone_id; ?>') {
            html += ' selected="selected"';
          }
