...
class ControllerSaleCustomer extends Controller {
	...
	protected function getForm() {
		...
		//echo "<pre>"; print_r($data['addresses']); exit;
		...
		foreach ($data['addresses'] as $key => &$value) {
			$index_key = $key;
			if(isset($data['addresses'][$index_key]['zone_id'])){
				$data['addresses'][$index_key]['cities'] = $this->model_localisation_city->getCitiesByZoneId($data['addresses'][$index_key]['zone_id']);
			}
		}
