...
//		$directories = glob($directory . '/' . $filter_name . '*', GLOB_ONLYDIR);
		...
		$icount = count($directories);
		for($i = 0; $i < $icount; $i++)
		{
			$directories[$i] = str_replace($directory . '/' . $filter_name .'/', '', $directories[$i]);
		}
		
		$directories = preg_grep('#'. $filter_name . '#i', $directories);
		...
		for($i = 0; $i < $icount; $i++)
		{
			$files[$i] = str_replace($directory . '/' . $filter_name .'/', '', $files[$i]);
		}
		
		$files = preg_grep('#'. $filter_name . '#i', $files);
